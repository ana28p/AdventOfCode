...
LLLLLLL.LLLLLLL..LLLLLLLLLLLLLLLLLLL.LLLLLLLLLLLLL.LLLLLLLLL.LL.LLLL.LLLLL.LLLLLLLLL.LLLLLLL.LLLLL""".splitlines(False)
...
LLLLLLLLLL
L.LLLLLL.L
L.LLLLL.LL""".splitlines(False)


def nr_of_occupied_adj(seats, seat_pos):
    print(seat_pos, len(seats), len(seats[0]))
    nr = 0
    i, j = seat_pos

    if seats[i - 1][j - 1] in '.L' and i > 0 and j > 0:
        nr += 1
    if seats[i - 1][j] in '.L' and i > 0:
        nr += 1
    if seats[i][j - 1] in '.L' and j > 0:
        nr += 1
    if seats[i + 1][j + 1] in '.L' and i < len(seats)-1 and j < len(seats[0])-1:
        nr += 1
    if seats[i + 1][j] in '.L' and i < len(seats)-1:
        nr += 1
    if seats[i][j + 1] in '.L' and j < len(seats[0])-1:
        nr += 1
    if seats[i - 1][j + 1] in '.L' and i > 0 and j < len(seats[0])-1:
        nr += 1
    if seats[i + 1][j - 1] in '.L' and j > 0 and i < len(seats)-1 and j > 0:
        nr += 1
    return nr


def count_occupied_adj(seats, seat_pos):
    nr = 0
    adjs = [-1, 0, 1]
    i, j = seat_pos
    for ai in adjs:
        for aj in adjs:
            if (ai == aj and ai == 0) or (i+ai) in [-1, len(seats)] or (j+aj) in [-1, len(seats[0])]:
                continue
            if seats[i + ai][j + aj] in "#":
                nr += 1
    return nr


def search_dir_steps(seats, start_pos, steps):
    i, j = start_pos
    incr_i, incr_j = steps
    while True:
        i = i + incr_i
        j = j + incr_j
        if i in [-1, len(seats)] or j in [-1, len(seats[0])]:
            break
        if seats[i][j] == "#":
            return True
        if seats[i][j] == "L":
...
def count_occupied_adj2(seats, seat_pos):
    nr = 0
    if search_dir_steps(seats, seat_pos, (0, 1)):
        nr += 1
    if search_dir_steps(seats, seat_pos, (1, 0)):
        nr += 1
    if search_dir_steps(seats, seat_pos, (1, 1)):
        nr += 1
    if search_dir_steps(seats, seat_pos, (0, -1)):
        nr += 1
    if search_dir_steps(seats, seat_pos, (-1, 0)):
...
def change_seats(data_input):
    changes = 0
    temp = copy.deepcopy(data_input[:])
    for i in range(len(data_input)):
        for j in range(len(data_input[i])):
            adj_seats = count_occupied_adj(data_input, (i, j))
            if data_input[i][j] == 'L' and adj_seats == 0:
                temp[i][j] = '#'
                changes += 1
            if data_input[i][j] == '#' and adj_seats >= 4:
                temp[i][j] = 'L'
                changes += 1
...
def change_seats2(data_input):
    changes = 0
    temp = copy.deepcopy(data_input[:])
    for i in range(len(data_input)):
        for j in range(len(data_input[i])):
            adj_seats = count_occupied_adj2(data_input, (i, j))
            if data_input[i][j] == 'L' and adj_seats == 0:
                temp[i][j] = '#'
                changes += 1
            if data_input[i][j] == '#' and adj_seats >= 5:
                temp[i][j] = 'L'
                changes += 1
...
def part1(data_input):
    ...
    for line in data_input:
        data1.append(list(line))

    change = True
    while change:
        data1, nr_chg = change_seats(data1)
        if nr_chg == 0:
            change = False
...
def part2(data_input):
    ...
    change = True
    while change:
        data1, nr_chg = change_seats2(data1)
        if nr_chg == 0:
            change = False
...
#.#.###
.##.##.""".splitlines(False)
    inp = [list(i.strip()) for i in inp]

    print(count_occupied_adj2(inp, (3, 3)))
