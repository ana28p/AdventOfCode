...
4-5 s: rsssw
2-14 c: jckbwnnlkcmvnwtj""".splitlines(False)


def split_in_two_by(value, delim):
    sections = value.split(delim, 1)
    return sections[0].strip(), sections[1].strip()


def split_input(value):
    policy, password = split_in_two_by(value, ':')
    l_h, letter = split_in_two_by(policy, ' ')
    low, high = split_in_two_by(l_h, '-')
...
if __name__ == "__main__":
    ...
    valid_p_2 = 0
    for i in input:
        l, h, ch, pw = split_input(i)
        ch_count = list(pw).count(ch)
        if (l <= ch_count) and (ch_count <= h):
            valid_p_1 += 1
        if ((pw[l-1] == ch) and (pw[h-1] != ch)) or ((pw[l-1] != ch) and (pw[h-1] == ch)):
            valid_p_2 += 1
