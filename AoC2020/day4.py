...
hgt:188cm hcl:#c0946f""".splitlines(False)
...
iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719""".splitlines(False)
    ...
    try:
        if key == 'byr':
            ...
            if value.startswith('#') and len(value) == 7:
                v = [i for i in value[1:] if i in 'abcdef0123456789']
                if len(v) == 6:
                    return True
        elif key == 'ecl':
        ...
        # if an int cast failed (not number) -> invalid
        return False

    return False


def valid_passport(passport_lines, expected_fields):
    passport = ' '.join(passport_lines)
    passport_data = passport.split(' ')
    present_keys = []
    valid = True
    for pair in passport_data:
        seq = pair.split(':', 1)
        present_keys.append(seq[0].lower())
        valid = valid and value_validation(seq[0].lower(), seq[1].lower())
    missing = [f for f in expected_fields if f not in present_keys]
    return len(missing) == 0 and valid


def get_valid_passports(input_data):
    passport_lines = []
    valid_passports = 0
    for line in input_data:
        if line.strip(' \n\t') == '':
            if valid_passport(passport_lines, required_fields):
                valid_passports += 1
            passport_lines = []
            continue
        passport_lines.append(line.strip(' \n\t'))
    # check the last passport too
    if valid_passport(passport_lines, required_fields):
        valid_passports += 1
...
if __name__ == "__main__":
    print(get_valid_passports(input))
