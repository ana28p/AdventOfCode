...
input = """faded yellow bags contain 4 mirrored fuchsia bags, 4 dotted indigo bags, 3 faded orange bags, 5 plaid crimson bags.
...
bright bronze bags contain 1 clear magenta bag, 2 clear gray bags, 2 dull coral bags, 5 vibrant gray bags.""".splitlines(False)
...
dotted black bags contain no other bags.""".splitlines(False)
...
dark violet bags contain no other bags.""".splitlines(False)
...
class Bag:
    def __init__(self, name):
        self.name = name
        self.in_bags = set()
        self.contains_bags = set()


class BagQuantity:
    def __init__(self, q, b, name):
        self.quantity = q
        self.bag = b
        self.bag_name = name
...
bags = {}
bags_quantities = {}  # str: List[BagQuantity]
bags_that_contain_shiny_gold = set()
bags_in_shiny_gold = set()
...
def create_bag(bag_info):
    current_bag = bag_info[:bag_info.find('contain')]
    name_parts = current_bag.strip(' ').split(' ')
    bag_name = name_parts[0] + name_parts[1]
    current_bag = get_bag(bag_name)

    quantities = []

    contain_part = bag_info[bag_info.find('contain') + 8:-1].strip(' ')
    if "no other bags" not in contain_part:
        contain_parts = contain_part.split(',')
        for contain_bag in contain_parts:
            parts = contain_bag.strip(' ').split(' ')
            quantity = int(parts[0])
            name = parts[1] + parts[2]

            c_bag = get_bag(name)
            c_bag.in_bags.add(current_bag)

            current_bag.contains_bags.add(c_bag)

            quantities.append(BagQuantity(quantity, c_bag, name))

    bags_quantities[bag_name] = quantities


def search_parent(current_bag):
    # print(current_bag.name)
    if len(current_bag.in_bags) == 0:
        return 0
    sum = 0
    for p_bag in current_bag.in_bags:
        bags_that_contain_shiny_gold.add(p_bag.name)
        bag = get_bag(p_bag.name)
        sum += len(bag.in_bags) + search_parent(bag)
    return sum


def calculate_bags_q(current_bag_name):
    if current_bag_name not in bags_quantities:
        return 0
    bag_qs = bags_quantities[current_bag_name]
    if len(bag_qs) == 0:
        return 0
    quantity = 0
    for bag_q in bag_qs:
        quantity += bag_q.quantity + (bag_q.quantity * calculate_bags_q(bag_q.bag_name))
    return quantity


def search_children(current_bag):
    if len(current_bag.contains_bags) == 0:
        return 0
    sum = 0
    for c_bag in current_bag.contains_bags:
        bags_in_shiny_gold.add(c_bag.name)
        bag = bags[c_bag.name]
        bag_qs = bags_quantities[c_bag.name]
        for b_q in bag_qs:
            sum += search_parent(bag)
...
def collect_data(data):
    ...
    #     is_in = [i.name for i in v.in_bags]
    #     contain = [i.name for i in v.contains_bags]
    #     print(k, 'contain', contain)
    #     print(k, 'is in', is_in)

    shiny_gold_bag = get_bag("shinygold")

    occ = search_parent(shiny_gold_bag)

    print('shiny gold bag can be in {} or {} bag(s)'.format(occ, len(bags_that_contain_shiny_gold)))

    print(calculate_bags_q("shinygold"))
